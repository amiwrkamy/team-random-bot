// index.js — Final robust random-team-bot (CommonJS)
'use strict';
...
const USE_WEBHOOK = (process.env.USE_WEBHOOK || (process.env.DOMAIN ? 'true' : 'false')).toLowerCase() === 'true';
...
group session:
{
  type:'group',
  teamsCount: number,
  teams: [ { members: [ {id,name,role} ], subs: [ {id,name,role} ] } ],
...
function assignPlayerToTeam(sess, userId, name) {
  ...
  // all teams full — put into subs on team with smallest subs
...
// reshuffle: reassign all existing members randomly while respecting 1 GK per team and max 5 per team
...
async function updateGroupMessage(chatId, sess) {
  ...
  try {
    ...
    // if edit fails (e.g., message not modified) — send new and store id
    console.error('updateGroupMessage failed (edit), sending new.', err && err.message);
    try {
      const sent = await bot.telegram.sendMessage(chatId, text, { parse_mode: 'HTML', reply_markup });
      sess.message_id = sent.message_id;
    } catch (e) {
      console.error('sending new message failed', e && e.message);
    }
  }
...
// health/readiness — Render needs at least one port open for Web Service
  ...
  try {
    if (USE_WEBHOOK && WEBHOOK_URL) {
      ...
      app.listen(PORT, async () => {
        ...
        try {
          // delete previous webhook, then set new
      ...
      // Polling mode: ensure no webhook active then launch polling
      try {
        await bot.telegram.deleteWebhook();
      ...
      // start a tiny HTTP server so Render sees an open port (prevents "no open ports" message)
      app.listen(PORT, () => {
